...
fn normalize_relative_path(path: &Path) -> PathBuf {
    ...
    for component in path.components() {
        match component {
            ...
            Component::ParentDir => {
                if normalized.file_name() == Some(std::ffi::OsStr::new("..")) || !normalized.pop() {
                    normalized.push("..");
                }
            }
            Component::RootDir | Component::Prefix(_) => unreachable!("relative path validated"),
        }
    }
    normalized
}
...
mod tests {
    ...
    fn temp_dir() -> PathBuf {
        let path = std::env::temp_dir().join(format!("mu-apply-patch-{}", uuid::Uuid::new_v4()));

...
Commands you can assume availability: POSIX commands (with GNU extension), `rg`, `jq`, `python`, `curl`, and `systemd` utilities. You can discover and use other software and services, but avoid installing new software without user agreement. Two special commands are available inside `bash`: `apply_patch` and `view_image`.

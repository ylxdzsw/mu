...
mod view_image {
    ...
    use crate::store::{
        ATTACHMENT_MANIFEST_ENV, BASH_CALL_ID_ENV, OBJECTS_DIR_ENV, stage_bash_attachment,
    };
    ...
    fn write_image_attachment(attachment: &Attachment, detail: ImageDetail) -> anyhow::Result<()> {
        let manifest = std::env::var_os(ATTACHMENT_MANIFEST_ENV)
        ...
        let objects_dir =
            std::env::var_os(OBJECTS_DIR_ENV).context("missing Mu object directory")?;

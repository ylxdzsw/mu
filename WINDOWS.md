...
Keep the Windows delta as one logical patch commit. That commit documents the
port, replaces Unix process, path, identity, randomness, and installation
behavior with native Windows implementations, and adds MSYS2 packaging and
tests. Optimize for a small conflict surface rather than artificial subsystem
boundaries. In particular, do not add a cross-platform abstraction layer solely
to retain code that this branch cannot run.
...
## 5. Attachment transport

`view_image.exe` uses the current direct SQLite attachment channel; it does not
inherit a Unix file descriptor or pass a native Windows handle through Bash.
For a live tool call, Mu exports the native session database path, Bash-call id,
and owning Mu PID. The applet validates the image, opens the scope database,
checks that the owner still owns the session and that the call remains open,
then inserts the attachment reference and content-addressed blob
transactionally.

The channel accepts at most eight images per call. It is unavailable outside a
live Mu Bash call. A descendant that writes after the direct call has committed
its result is rejected; detached work must save a normal file for a later
foreground `view_image` invocation. Session-owner liveness is checked with
Win32 process APIs rather than Unix signals.
...
Windows file locking, read-only attributes, and symlink behavior differ from
Unix and need explicit
handling in `apply_patch` and `edit`:

- Updating an existing file takes a non-blocking lock, verifies the preflight
  snapshot, writes and syncs a sibling recovery backup, updates and syncs the
  existing file, then removes the backup. A read-only attribute is cleared only
  for the update and restored afterward.
- Add operations must retain their no-overwrite guarantee.
- Temporary and recovery files are created beside their destination.
...
Tests must cover update of an existing file, no-overwrite creation,
...
cryptographic randomness. Provider HTTPS uses Windows SChannel through
...
`src/store.rs`, `src/install.rs`, `src/random.rs`, `src/applets.rs`,
`Cargo.toml`, `README.md`, and `SPEC.md` are expected conflict hotspots.
...
- Windows cancellation prioritizes guaranteed tree cleanup over a SIGTERM-like
  grace period.
- Direct SQLite attachment insertion replaces inherited artifact descriptors.
